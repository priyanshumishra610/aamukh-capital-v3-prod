...
export default function TeamExpertiseSection() {
  ...
  return (
    <section 
      id="team" 
      ref={containerRef}
      className="page-surface w-full px-[6vw] lg:px-[4vw] py-16 md:py-20 border-t border-border/50 overflow-hidden"
    >
      ...
      <div className="relative z-10 w-full max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6 border-b border-border pb-8">
          ...
          <motion.div 
            initial={{ opacity: 0, x: 20 }}
            animate={isInView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.5, delay: 0.08, ease: cinematicEase }}
        ...
        <div className="flex flex-col relative w-full group/list">
           ...
           {OPERATORS.map((person, index) => (
             <motion.div 
                ...
                transition={{ duration: 0.42, delay: 0.12 + (index * 0.06), ease: cinematicEase }}
                onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
                className="group relative flex flex-col md:flex-row justify-between border-b border-border py-12 md:py-16 hover:bg-white/40 transition-colors duration-200 cursor-pointer px-4 lg:pl-12"
             >
